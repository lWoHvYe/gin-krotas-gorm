...
func newRole(db *gorm.DB, opts ...gen.DOOption) role {
	...
	_role.UserRoles = roleHasManyUserRoles{
...
type role struct {
	...
	Code      field.String
	Name      field.String
	UserRoles roleHasManyUserRoles
...
func (r role) clone(db *gorm.DB) role {
	...
	r.UserRoles.db = db.Session(&gorm.Session{Initialized: true})
	r.UserRoles.db.Statement.ConnPool = db.Statement.ConnPool
	return r
}

func (r role) replaceDB(db *gorm.DB) role {
	r.roleDo.ReplaceDB(db)
	r.UserRoles.db = db.Session(&gorm.Session{})
	return r
}

type roleHasManyUserRoles struct {
	db *gorm.DB
...
func (a roleHasManyUserRoles) WithContext(ctx context.Context) *roleHasManyUserRoles {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a roleHasManyUserRoles) Session(session *gorm.Session) *roleHasManyUserRoles {
	a.db = a.db.Session(session)
	return &a
}

func (a roleHasManyUserRoles) Model(m *model.Role) *roleHasManyUserRolesTx {
	return &roleHasManyUserRolesTx{a.db.Model(m).Association(a.Name())}
}

func (a roleHasManyUserRoles) Unscoped() *roleHasManyUserRoles {
	a.db = a.db.Unscoped()
	return &a
}

type roleHasManyUserRolesTx struct{ tx *gorm.Association }

func (a roleHasManyUserRolesTx) Find() (result []*model.UserRole, err error) {
	return result, a.tx.Find(&result)
}

func (a roleHasManyUserRolesTx) Append(values ...*model.UserRole) (err error) {
...
func (a roleHasManyUserRolesTx) Replace(values ...*model.UserRole) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a roleHasManyUserRolesTx) Delete(values ...*model.UserRole) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a roleHasManyUserRolesTx) Clear() error {
	return a.tx.Clear()
}

func (a roleHasManyUserRolesTx) Count() int64 {
	return a.tx.Count()
}

func (a roleHasManyUserRolesTx) Unscoped() *roleHasManyUserRolesTx {
	a.tx = a.tx.Unscoped()
	return &a
}
